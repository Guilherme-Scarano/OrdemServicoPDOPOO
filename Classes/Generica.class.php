...
class Generica
{
    ...
    public function editarDados($tabela, $codigo, $status, $data) {
        try {
            $this->tabela = $tabela;
            $this->codigo = $codigo;
            $this->status = $status;
            ...
            $verificarDados = $conexao->prepare("SELECT * FROM $this->tabela WHERE cod = ?");
            $verificarDados->bindParam(1, $codigo);
            $verificarDados->execute();
            $retorno = $verificarDados->fetch(PDO::FETCH_ASSOC);
        
            if (count($retorno) > 0) {
                // Atualizar o registro
                $atualizarDados = $conexao->prepare("UPDATE $this->tabela SET status = ?, data = ? WHERE cod = ?");
                $atualizarDados->bindParam(1, $status);
                $atualizarDados->bindParam(2, $data);
                $atualizarDados->bindParam(3, $codigo);
                $retorno = $atualizarDados->execute(); // Retorna true ou false
